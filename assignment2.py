...
# YOUR CODE HERE
def extract_temps(temps): #takes arguement temps
    new = [] # empty array
    for i in range(len(temps)): #converting items from a string into a float
        try:
            new.append(float(temps[i]))
        except:
            pass
    return new

def selection_sort(nums): #takes arguement nums
    new_arr = nums.copy() #duplicate list
    for i in range(len(new_arr)): #applying the selection sort to the array, then go through the elements in the array
        minimum_index = i #find minimum element in unsorted array
        for j in range(i+1, len(new_arr)):
            if new_arr[minimum_index] > new_arr[j]:
                minimum_index = j
        new_arr[i], new_arr[minimum_index] = new_arr[minimum_index], new_arr[i] #swap minimum element with the first element
    return new_arr

def calculate_median(nums): #takes arguement nums
    if len(nums) <= 0:
        return
    if len(nums) % 2 == 0: #determining odd or even length of list
        n = len(nums)//2 
        median = (nums[n] + nums[n-1])/2 #if even do integer division between 2 numbers
    else:
        n = len(nums)//2 #if odd do integer division on sole number
        median = nums[n]
    return median

def main(temp_data): #takes arguement temp_data
    try:
        nums = extract_temps(temp_data) #string of integer to float
        if len(nums) == 0:
            return "N/A"
        new_arr = selection_sort(nums) #sort the function
        median = calculate_median(new_arr) #calculate the median
        return median
    except:
        return "N/A" #If error occurs print
...
# Input: temps_data = ["Candice", "Evergreen", "Derek"]
# Expected Output: N/A
# Pass/Fail: Pass
# 
# Boundary
...
# ```
# 1. If the list is all empty strings, or letters, containing zero numbers.
# 2. No, because if they are held in the main function it would be unnecessary to reiterate a try or except statement due to the fact the function is called there. 
# ```
